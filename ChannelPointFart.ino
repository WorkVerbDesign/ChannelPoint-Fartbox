...
const uint16_t farTime[9] = {1430, 2050, 1140, 1260, 750, 900, 2340, 1390, 450};
...
void setupPins(){
  ...
  for(uint8_t i = 0; i < fartSize; i++){
    //tx/rx need a special mode set, 12 13 14 too || farts[i] == 12 || farts[i] == 13 || farts[i] == 14
    if(farts[i] == 1 || farts[i] == 3){
      //(we use Tx/Rx pins so we have to set pinmode of 1 and 3 to Function_3)
      pinMode(farts[i], FUNCTION_3);
...
void testPorts(){
  for(int i=0; i < fartSize; i++){
    digitalWrite(farts[i], HIGH);
    delay(500);
    digitalWrite(farts[i], LOW);
  }
}
